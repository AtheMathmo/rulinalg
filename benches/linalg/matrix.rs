...
fn mat_mat_elemul_63_1000(b: &mut Bencher) {
    ...
    b.iter(|| utils::vec_bin_op(a.data(), c.data(), f32::mul))
}
...
fn mat_sum_rows_and_cols_128_100(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        assert_eq!(sum_rows.data(),
                   &(0..100).map(|i| i * 128).collect::<Vec<_>>());
...
fn mat_swap_cols_0_99(b: &mut Bencher) {
    ...
    b.iter(|| { black_box(m.swap_cols(0, 99)); });
}
...
fn mat_mean_cols(b: &mut Bencher) {
    ...
    b.iter(|| { black_box(m.mean(Axes::Col)); });
}
...
fn mat_mean_rows(b: &mut Bencher) {
    ...
    b.iter(|| { black_box(m.mean(Axes::Row)); });
}
...
fn mat_min_col(b: &mut Bencher) {
    ...
    b.iter(|| { black_box(m.min(Axes::Col)); });
}
...
fn mat_min_row(b: &mut Bencher) {
    ...
    b.iter(|| { black_box(m.min(Axes::Row)); });
}
