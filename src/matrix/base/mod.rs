...
pub trait BaseMatrix<T>: Sized {
        ...
        where T: 'a
    {
        ...
        let offset = (start[0] * self.row_stride() + start[1]) as isize;

        unsafe {
            MatrixSlice::from_raw_parts(self.as_ptr().offset(offset), rows, cols, self.row_stride())
        }
    }
}
...
pub trait BaseMatrixMut<T>: BaseMatrix<T> {
        ...
        where T: 'a
    {
        ...
        let count = (start[0] * self.row_stride() + start[1]) as isize;

        unsafe {
            MatrixSliceMut::from_raw_parts(self.as_mut_ptr().offset(count),
                                           rows,
                                           cols,
                                           self.row_stride())
        }
