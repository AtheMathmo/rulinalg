...
impl<T: Any + Float + Signed> Matrix<T> {
    ...
    /// values are ordered in non-increasing order. U and V have orthonormal columns, and
    /// each column represents the left and right singular vectors for the corresponding
    /// singular value in Σ, respectively.
    ///
    ...
    /// efficient is fairly basic. Hopefully the algorithm can be made not to fail in the
    /// near future.
...
mod tests {
    ...
    fn validate_svd(mat: &Matrix<f64>, b: &Matrix<f64>, u: &Matrix<f64>, v: &Matrix<f64>) {
        ...
        let mut singular_triplets = u_transposed.row_iter()
            .zip(b.diag()).zip(v_transposed.row_iter())
    ...
    fn test_svd_short_matrix() {
        ...
        // the "natural" order of the singular values it not sorted by default.
        let mat = matrix![
            3.61833700244349288,  3.96046289599926427,  1.44435028724617442,  1.13455445826500645,
            -3.30895752093770579;

            -3.28382346228211697, 0.70730060716580723,  1.27749196276785826,  0.81521390274755756,
            -0.04979044289857298;

            1.97968027781346545,  -2.80552479438772817, -1.09858397535426366, 3.99123446373437263,
            3.03248594516832792;

            -0.41869628192662156, -1.45283286109873933, -0.03159619816434689, -2.83025703359666192,
            3.85962479743330977
        ];
