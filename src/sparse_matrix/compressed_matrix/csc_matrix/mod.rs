...
use sparse_matrix::{CsrMatrix, CompressedIter, CompressedIterMut, CompressedMatrix,
                    SparseMatrix};
...
impl<T: Copy + One + Zero> CompressedMatrix<T> for CscMatrix<T> {
    ...
    fn from_triplets(rows: usize, cols: usize, triplets: &[(usize, usize, T)]) -> CscMatrix<T>
    {
...
mod tests {
    ...
    fn test_from_triplets() {
        let a = CscMatrix::from_triplets(3, 3, &[(0, 0, 1), (1, 1, 2), (2, 2, 3), (1, 1, 3)]);
        let b = CscMatrix::new(3, 3, vec![1, 5, 3], vec![0, 1, 2], vec![0, 1, 2, 3]);
        assert_eq!(a, b);
    }
    #[test]
    fn test_identity() {
