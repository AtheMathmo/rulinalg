...
use sparse_matrix::{CscMatrix, CompressedIter, CompressedIterMut, CompressedMatrix,
                    SparseMatrix};
...
impl<T: Copy + One + Zero> CompressedMatrix<T> for CsrMatrix<T> {
    ...
    fn from_triplets(rows: usize, cols: usize, triplets: &[(usize, usize, T)]) -> CsrMatrix<T>
    {
...
mod tests {
    ...
    fn test_from_triplets() {
        let a = CsrMatrix::from_triplets(3, 3, &[(0, 0, 1), (1, 1, 2), (2, 2, 3), (1, 1, 8)]);
        let b = CsrMatrix::new(3, 3, vec![1, 10, 3], vec![0, 1, 2], vec![0, 1, 2, 3]);
        assert_eq!(a, b);
    }
    #[test]
    fn test_identity() {
