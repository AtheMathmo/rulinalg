...
/// (../macro.assert_matrix_eq!.html).
///
/// Usually you should not need to interface with this trait directly. It is a part of
/// the documentation only so that the trait bounds for the comparators are made public.
...
/// # fn main() { let x = matrix![1.0f64]; let y = matrix![1.0f64];
...
/// The `float` comparator is designed to be a conservative default for comparing
/// floating-point numbers.
/// It is inspired by the `AlmostEqualUlpsAndAbs` comparison function proposed in
/// the excellent blog post [Comparing Floating Point Numbers, 2012 Edition]
...
///
/// Note that the ULP difference of two floating point numbers is not defined in the
/// following cases:
///
...
///
/// Note that the scalar type of the matrix must implement the [Ulp trait](ulp/trait.Ulp.html)
/// in order to be used with this comparator. By default, `f32` and `f64` implementations
/// are provided.
///
/// # Error reporting
///
...
macro_rules! assert_matrix_eq {
    ($x:expr, $y:expr) => {
        {
            // Note: The reason we take slices of both x and y is that if x or y are passed
            // as references, we don't attempt to call elementwise_matrix_comparison with
            // a &&BaseMatrix type (double reference), which does not work due to generics.
            ...
            let msg = elementwise_matrix_comparison(&$x.as_slice(), &$y.as_slice(), comp)
                .panic_message();
            if let Some(msg) = msg {
...
macro_rules! assert_vector_eq {
    ($x:expr, $y:expr) => {
        {
            // Note: The reason we take slices of both x and y is that if x or y are passed
            // as references, we don't attempt to call elementwise_matrix_comparison
            // with a &&BaseMatrix type (double reference), which does not work due to generics.
            use $crate::macros::{elementwise_vector_comparison, ExactElementwiseComparator};
            let comp = ExactElementwiseComparator;
            let msg = elementwise_vector_comparison($x.data(), $y.data(), comp)
                .panic_message();
            if let Some(msg) = msg {
                // Note: We need the panic to incur here inside of the macro in order
                // for the line number to be correct when using it for tests,
...
mod tests {
    ...
    quickcheck! {
        fn property_ulp_comparator_next_f64_is_ok_when_inside_tolerance(x: f64) -> TestResult {
            ...
            let err = Err(UlpError(UlpComparisonResult::Difference(1)));

            let tol_0_fails = comp0.compare(x, y) == err;
    ...
    quickcheck! {
        fn property_float_comparator_matches_abs_with_zero_ulp_tol(a: f64, b: f64, abstol: f64)
            -> TestResult {
            if abstol <= 0.0 {
                return TestResult::discard()
            }
    ...
    quickcheck! {
        fn property_float_comparator_matches_ulp_with_zero_eps_tol(a: f64, b: f64, max_ulp: u64)
            -> bool {
    ...
    quickcheck! {
        fn property_elementwise_comparison_incompatible_matrices_yield_dimension_mismatch(
            ...
            let expected = MatrixComparisonResult::MismatchedDimensions {
                dim_x: (m, n), dim_y: (p, q)
            };
    ...
    quickcheck! {
        fn property_elementwise_vector_comparison_incompatible_vectors_yields_dimension_mismatch(
            ...
            let comparasion = elementwise_vector_comparison(x.data(), y.data(), comp);
            let expected = VectorComparisonResult::MismatchedDimensions { dim_x: m, dim_y: n };

            TestResult::from_bool(comparasion == expected)
        }
    }
