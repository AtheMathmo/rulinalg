...
/// Floating point types for which two instances can be compared for Unit in the Last Place (ULP)
/// difference.
///
...
///
/// A somewhat accessible (but not exhaustive) guide on the topic is available in the popular
...
/// need to implement this. In the case when a custom implementation is necessary, please see the
/// possible return values for [UlpComparisonResult](ulp/enum.UlpComparisonResult.html).
/// Otherwise, we can recommend to read the source code of the included `f32` and `f64`
/// implementations.
pub trait Ulp {
        ...
        impl Ulp for $ftype {
            fn ulp_diff(a: &Self, b: &Self) -> UlpComparisonResult {
                if a == b {
                    ...
                    // Otherwise, we compute the ULP diff as the difference of the signed integer
                    // representations
...
mod tests {
    ...
    quickcheck! {
        fn property_recovers_ulp_diff_when_f32_constructed_from_i32(a: i32, b: i32) -> TestResult {
            ...
            if x.is_finite() && y.is_finite() && x.signum() == y.signum() {
                let difference = UlpComparisonResult::Difference((b - a).abs() as u64);
                TestResult::from_bool(f32::ulp_diff(&x, &y) == difference)
            } else {
                TestResult::discard()
            }
        }
    ...
    quickcheck! {
        fn property_recovers_ulp_diff_when_f64_constructed_from_i64(a: i64, b: i64) -> TestResult {
            ...
            if x.is_finite() && y.is_finite() && x.signum() == y.signum() {
                let difference = UlpComparisonResult::Difference((b - a).abs() as u64);
                TestResult::from_bool(f64::ulp_diff(&x, &y) ==  difference)
            } else {
                TestResult::discard()
            }
        }
    ...
    quickcheck! {
        fn property_f32_incompatible_signs_yield_corresponding_enum_value(x: f32, y: f32)
        -> TestResult {
            if x.signum() == y.signum() {
                ...
                let diff = f32::ulp_diff(&x, &y);
                TestResult::from_bool(diff == UlpComparisonResult::IncompatibleSigns)
            }
        }
    }

    quickcheck! {
        fn property_f64_incompatible_signs_yield_corresponding_enum_value(x: f64, y: f64)
        -> TestResult {
            if x.signum() == y.signum() {
                TestResult::discard()
            } else if x.is_nan() || y.is_nan() {
